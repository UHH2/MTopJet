...
int main(int argc, char* argv[]){
  ...
  // TFile* file = new TFile("/nfs/dust/cms/user/schwarzd/CMSSW_8_0_24_patch1/src/UHH2/MTopJet/Unfolding_Run2/Results_data_combine.root");
  TFile* file = new TFile("/nfs/dust/cms/user/schwarzd/CMSSW_8_0_24_patch1/src/UHH2/MTopJet/Unfolding_Run2/Results_data_muon.root");

  TH1F * h_data = (TH1F*)file->Get("Unfold_XS_totuncert");
  TH1F * h_data_stat = (TH1F*)file->Get("Unfold_XS_statuncert");
  TH1F * h_truth = (TH1F*)file->Get("Truth_XS_error");
  TH1F * h_truth2 = (TH1F*)file->Get("Truth2_XS");
  ...
  TH1F * h_ratio_amc = GetRatio(h_truth2, h_data);

  std::vector<TH1D*> truth, truth2;
  for(auto t: {h_truth, h_truth2}){
    truth.push_back( (TH1D*) t->Clone() );
    truth2.push_back( (TH1D*) t->Clone() );
  }
  vector<TString> legnames = {"POWHEG", "MG5_aMC@NLO"};
  ...
  h_data->GetYaxis()->SetTitle("#frac{d#sigma}{d#it{m}_{jet}} #left[#frac{fb}{GeV}#right]");
  ...
  TString infotext = "140 fb^{-1} (13 TeV)";
  ...
  c->SaveAs("/afs/desy.de/user/s/schwarzd/Plots/Unfolding_Run2/Unfold.pdf");
  delete c;
}
